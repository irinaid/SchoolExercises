...
rotated_tulips.save("rotated45.jpg", "JPEG")

# Rotate the image upside down
upside_down = tulips.rotate(90).rotate(90)
upside_down.save("upside_down.jpg", "JPEG")
...
for i in range(0, len(photos_album)):
  ...
  image.save("album45/image" + str(i) + ".jpg", "JPEG")
  album45.append(image)

os.mkdir("rotated_pics")
album180 = []
# Iterate over the list to make other transformations!
for i in range(0, len(photos_album)):
  image = photos_album[i].rotate(180)
  image.save("rotated_pics/image" + str(i) + ".jpg", "JPEG")
  album180.append(image)

os.mkdir("flipped_pics")
albumflipped = []
for i in range(0, len(photos_album)):
  image = photos_album[i].transpose(Image.FLIP_LEFT_RIGHT)
  image.save("flipped_pics/image" + str(i)  + ".jpg", "JPEG")
